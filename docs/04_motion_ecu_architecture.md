...
Motion ECU는 **UDS responder + FOTA target**이다. Gateway로부터 CAN FD로 UDS 요청을 받아 `Dcm`이 처리하고, FOTA 관련 서비스는 `FotaHandler`→`Sota*` 코어로 위임해 inactive PFlash bank에 기록·검증·스왑한다.
...
flowchart TD
    ...
    M --> DCM["Dcm_MainFunction"]
    M --> CANW["Can_MainFunction_Write"]
    M --> FH["FOTAHandlerMain"]
...
근거:
- `Test_InitModules()`/`Test_MainFunctions()` in `Ecu_Motion_TC375_LK/Cpu0_Main.c` (Can→CanIf→CanTp→PduR→Dcm→`FOTA_ProvisionInitialOnce`)
...
flowchart TD
    ...
    W --> F["FOTAHandlerMain (chunk→PFlash)"]
    F --> DLY["Shared_Util_Time_DelayMs(1)"]
    DLY --> S
```

> FOTA의 실제 flash 기록은 `Dcm`이 아니라 **main loop의 `FOTAHandlerMain()`**에서 수행된다. Dcm은 chunk를 등록하고 `DCM_WRITE_PENDING`을 반환하여 다음 main cycle에서 처리되게 한다. 책임 분리는 [DCM Diagnostic Services](./14_dcm_diagnostic_services.md) 참고.
...
| 항목 | 공통 여부 | 비고 |
|---|---|---|
| `Dcm.c`, `FotaHandler.c`, `CanTp_Cfg.c` | **동일** | `diff` 결과 차이 없음 |
| `Cpu0_Main.c` | 차이 | UART 배너 `[Motion]` vs `[Lighting]` |
| `Can_Cfg.h` | 차이 | CAN ID 0x500/0x501 vs 0x600/0x601 |

근거: `diff Ecu_Motion_TC375_LK/... Ecu_Lighting_TC375_LK/...` (Dcm/FotaHandler/CanTp_Cfg 동일, Cpu0_Main/Can_Cfg.h 차이).
